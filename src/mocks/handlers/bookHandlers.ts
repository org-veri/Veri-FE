...
export const bookHandlers = [
  http.get('*/api/bookshelf/my', async ({ request }) => {
  ...
  http.get('*/api/bookshelf/:memberBookId', async ({ params }) => {
  ...
  http.get('*/api/bookshelf/search', async ({ request }) => {
    const url = new URL(request.url);
  ...
  http.get('*/api/bookshelf/popular', async ({ request }) => {
    const url = new URL(request.url);
    const page = Number(url.searchParams.get('page') || '1');
    const size = Number(url.searchParams.get('size') || '10');
  ...
  http.get('*/api/bookshelf/recommendation/today', async () => {
    const body = createMockResponse(mockTodaysRecommendation, 'Mock 오늘의 추천 도서 조회 성공');
    return withDelay(body);
  }),
  http.post('*/api/bookshelf', async ({ request }) => {
  ...
  http.delete('*/api/bookshelf/:memberBookId', async ({ params }) => {
    const id = Number(params.memberBookId);
  ...
  http.post('*/api/bookshelf/:memberBookId/start', async ({ params }) => {
    const body = createMockResponse({ id: Number(params.memberBookId) }, 'Mock 독서 시작 성공');
    return withDelay(body);
  }),
  http.post('*/api/bookshelf/:memberBookId/over', async ({ params }) => {
    const body = createMockResponse({ id: Number(params.memberBookId) }, 'Mock 독서 완료 성공');
    return withDelay(body);
  }),
  http.patch('*/api/bookshelf/:memberBookId/rate', async ({ params, request }) => {
    const id = Number(params.memberBookId);
  ...
  http.get('*/api/bookshelf/my/count', async () => {
    const body = createMockResponse({ count: books.length }, 'Mock 내 책 개수 조회 성공');
    return withDelay(body);
  }),
  http.get('*/api/bookshelf/my/search', async ({ request }) => {
    const url = new URL(request.url);
  ...
  http.patch('*/api/bookshelf/:readingId/visibility', async ({ params, request }) => {
    const url = new URL(request.url);
